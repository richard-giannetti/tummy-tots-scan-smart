          ...
          {navItems.map((item) => {
            ...
            if (item.isSpecial) {
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center p-2 rounded-xl transition-all duration-300 transform hover:scale-110 ${
                    item.active
                ...
                >
                  <div className="w-7 h-7 bg-white/20 rounded-full flex items-center justify-center mb-1 backdrop-blur-sm">
                    <IconComponent className="w-4 h-4" />
                  </div>
                  <span className="text-xs font-bold tracking-wide">{item.label}</span>
                  {/* Enhanced glow effect */}
                  <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 opacity-20 blur-md -z-10"></div>
                </button>
              );
            }
            ...
            return (
              <button
                key={item.route}
                onClick={() => handleNavigation(item.route)}
                className={`group flex flex-col items-center p-2 rounded-lg transition-all duration-300 min-w-[50px] ${
                  item.active
