          ...
          {navItems.map((item) => {
            ...
            if (item.isSpecial) {
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center p-1.5 rounded-lg transition-all duration-300 transform hover:scale-110 ${
                    item.active
                ...
                >
                  <div className="w-5 h-5 bg-white/20 rounded-full flex items-center justify-center mb-0.5 backdrop-blur-sm">
                    <IconComponent className="w-3 h-3" />
                  </div>
                  <span className="text-[10px] font-bold tracking-wide">{item.label}</span>
                  {/* Enhanced glow effect */}
                  <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 opacity-20 blur-md -z-10"></div>
                </button>
              );
            }
            ...
            return (
              <button
                key={item.route}
                onClick={() => handleNavigation(item.route)}
                className={`group flex flex-col items-center p-1.5 rounded-md transition-all duration-300 min-w-[42px] ${
                  item.active
              ...
              >
                <div className={`p-0.5 rounded-md transition-all duration-300 ${
                  item.active 
